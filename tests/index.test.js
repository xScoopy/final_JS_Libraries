...
test("short phone number error", () => {
    expect(() => { formatPhone(shortNumber)}).toThrow("Phone number was too short")
})
test("long phone number error", () => {
    expect(() => { formatPhone(longNumber)}).toThrow("Phone number too long")
})
test("int input instead of string", () => {
    expect(() => { formatPhone(1234567891)}).toThrow("Please provide phone number in string format")
})